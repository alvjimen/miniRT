...
/*   Updated: 2023/05/26 13:42:55 by alvjimen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "minirt.h"
...
void	ft_img(t_data *img, const int image_width, const double aspect_ratio)
{
	...
	img->mlx = mlx_init();
	img->mlx_win  = mlx_new_window(img->mlx, image_width, image_height, "miniRT");
...
t_camera	*ft_init_camera(t_vec3d *origin, const double aspect_ratio)
{
	...
	ptr->lower_left_corner = ft_init_vec3d(origin->x - (ptr->viewport_width / 2),
			origin->y - (ptr->viewport_height / 2), origin->z - ptr->focal_length);
	return (ptr);
}
